...
// Enhanced interface for editable position data
interface EditablePositionData {
    position: string;
    company: string;
    email: string;
    cphone?: string;
...
// Edit mode types based on schema
type EditMode = 'edit' | 'locked' | 'readonly';

class EditablePositionsDataGrid extends DataGrid {
    private _editMode: EditMode = 'readonly';
    private _editingRowIndex: number = -1;
    ...
    protected render(): void {
        ...
        const visibleFields = Object.keys(schema).filter(fieldName => {
            const field = schema[fieldName] as any;
    ...
    private createHeaderRow(visibleFields: string[], schema: any): string {
        return visibleFields.map(field => {
            const fieldSchema = schema[field] as any;
            const displayName = fieldSchema.displayName || field;
            return `<th data-field="${field}">${displayName}</th>`;
        }).join('');
    }

    private createDataRows(data: any[], visibleFields: string[], schema: any): string {
    ...
    private createCell(record: any, field: string, fieldSchema: any, isEditing: boolean, rowIndex: number): string {
    ...
    private createEditableCell(field: string, value: string, fieldSchema: any, rowIndex: number): string {
    ...
    private createActionCell(rowIndex: number, isEditing: boolean): string {
    ...
    private getEditableAttribute(fieldSchema: any): EditMode {
        // Check schema for edit permissions
        if (fieldSchema.locked === true) return 'locked';
        if (fieldSchema.editable === false) return 'readonly';
        if (fieldSchema.edit === true || this._editMode === 'edit') return 'edit';
        return 'readonly';
    }

    private getCellClass(editableAttr: EditMode): string {
        switch (editableAttr) {
    ...
    private getInputType(fieldSchema: any): string {
        return fieldSchema.htmlType || fieldSchema.type || 'text';
    }

    private attachEventListeners(): void {
        // Edit mode selector
        const editModeSelect = this.querySelector('.edit-mode-select') as HTMLSelectElement;
        if (editModeSelect) {
            editModeSelect.addEventListener('change', (e) => {
                this.editMode = (e.target as HTMLSelectElement).value as EditMode;
            });
        }
        ...
        this.querySelectorAll('.edit-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const rowIndex = parseInt((e.target as HTMLElement).dataset.row || '0');
    ...
    private startEditing(rowIndex: number): void {
        ...
        inputs.forEach(input => {
            ...
            if (field) {
                if (input.type === 'checkbox') {
                    updatedData[field] = (input as HTMLInputElement).checked;
                } else {
                    updatedData[field] = (input as HTMLInputElement).value;
                }
            }
        });
